...
export default function Dashboard({ records, filters, stats }) {
    ...
    // Carregar GeoJSON de forma assíncrona para não travar o carregamento inicial
    useEffect(() => {
                ...
                <div className="lg:col-span-3 bg-slate-900/50 rounded-[2rem] border border-white/5 overflow-hidden shadow-3xl relative backdrop-blur-sm group flex flex-col">
                    <div className="flex-1 relative">
                        ...
                        >
                            ...
                            {geoError && (
                                <div className="absolute inset-0 flex items-center justify-center bg-slate-950/80 z-[2000] backdrop-blur-sm p-8 text-center">
                                    <div className="max-w-xs">
                                        <ShieldAlert size={48} className="text-red-500 mx-auto mb-4" />
                                        <h3 className="text-lg font-bold text-white mb-2">Erro no Mapa</h3>
                                        <p className="text-sm text-slate-400 mb-6">{geoError}</p>
                                        <button 
                                            onClick={() => window.location.reload()}
                                            className="px-6 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl border border-white/5 transition-colors text-xs font-bold uppercase tracking-widest"
                                        >
                                            Recarregar Página
                                        </button>
                                    </div>
                                ...
                                <div className="bg-slate-900/95 backdrop-blur-2xl p-8 rounded-3xl border border-white/10 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.8)] relative overflow-hidden group">
                                    ...
                                    <div className="flex gap-12">
                                        <div className="flex-none w-1/3">
                                            ...
                                            <div className="flex-1 bg-slate-950/40 p-5 rounded-2xl border border-white/5 flex flex-col" style={{ minHeight: '300px' }}>
                                                <h3 className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-4 flex items-center gap-2">
                                                    <Activity size={12} className="text-emerald-500" />
                                                    Evolução nas últimas 12 semanas
                                                </h3>
                                                <div className="flex-1 w-full" style={{ position: 'relative', height: '200px' }}>
                                                    {history.length > 0 ? (
                                    ...
                                    <div className="min-w-0 text-left">
                                        ...
                                        <div className="flex items-center gap-3 mt-1.5">
                                            <div className="flex flex-col">
                                                <span className="text-[8px] text-slate-500 font-black uppercase tracking-widest">Total 2026</span>
                                                <span className="text-[10px] text-slate-300 font-bold">{(item.total_cases || 0).toLocaleString()}</span>
                                            </div>
                                            <div className="w-px h-4 bg-white/10" />
                                            <div className="flex flex-col">
                                                <span className="text-[8px] text-orange-500/70 font-black uppercase tracking-widest">Novos</span>
                                                <span className="text-[10px] text-orange-400 font-bold">{(isStateView ? (item.new_cases || 0) : (item.cases || 0)).toLocaleString()}</span>
                                            </div>
                                        </div>
                                    </div>
