...
export default function Dashboard({ records, filters, stats }) {
  ...
  useEffect(() => {
    ...
    const zoom = filters.uf ? 7 : 4;
    const url = filters.uf
      ? `/api/geo/map?uf=${filters.uf}&zoom=${zoom}`
      : '/data/brazil-states.json';
  ...
  useEffect(() => {
    if (selectedRecord) {
      ...
      axios
        .get(`/api/history/${cityId}?disease=${disease}`)
        .then((res) => {
          setHistory(res.data);
  ...
  const sortedDataList = useMemo(() => {
    return [...filteredDataList].sort(
      (a, b) => (b.total_cases || b.cases || 0) - (a.total_cases || a.cases || 0),
    );
  }, [filteredDataList]);

  const recordsDataMap = useMemo(() => {
    if (!Array.isArray(dataList)) return {};
    return dataList.reduce((acc, curr) => {
      const key = isStateView ? curr.uf : curr.city?.ibge_code || curr.ibge_code;
      if (key) acc[String(key)] = curr;
      return acc;
    }, {});
  ...
  const geoJsonStyle = useCallback(
    (feature) => {
      const key = isStateView ? feature.properties.sigla : feature.properties.ibge_code;
      const data = recordsDataMap[String(key)];
      const isHovered = hoveredUf === key;

      return {
        fillColor: data
          ? getLevelColor(data.level, isHovered ? 0.6 : 0.2)
          : 'rgba(255, 255, 255, 0.05)',
        weight: isHovered ? 2 : 0.5,
        opacity: 1,
        color: isHovered ? '#fff' : 'rgba(255,255,255,0.1)',
        fillOpacity: data ? (isHovered ? 0.6 : 0.2) : 0,
      };
    },
    [recordsDataMap, hoveredUf, isStateView],
  );
  ...
  const onEachFeature = useCallback(
    ...
    [isStateView, handleStateClick, recordsDataMap],
  );

  const pointToLayer = useCallback(() => null, []);
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="space-y-2 text-center">
              ...
              <p className="text-slate-500 text-sm font-medium">
                Sincronizando inteligência epidemiológica...
              </p>
            </div>
          </div>
        </div>
      ...
      <React.Suspense
        fallback={
          <div className="h-full w-full bg-slate-950 flex flex-col items-center justify-center">
            ...
            <span className="text-[10px] text-slate-700 font-black tracking-[0.3em] uppercase">
              Inicializando Sistema...
            </span>
          </div>
        }
      >
        <ControlHeader filters={filters} handleBack={handleBack} stats={stats} />
